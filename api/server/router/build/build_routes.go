...
func newImageBuildOptions(ctx context.Context, r *http.Request) (*types.ImageBuildOptions, error) {
	...
	if versions.GreaterThanOrEqualTo(version, "1.32") {
		...
		if err := system.ValidatePlatform(p); err != nil {
			return nil, errclass.InvalidArgument(errors.Errorf("invalid platform: %s", err))
		}
	...
	if i := container.Isolation(r.FormValue("isolation")); i != "" {
		if !container.Isolation.IsValid(i) {
			return nil, errclass.InvalidArgument(errors.Errorf("Unsupported isolation: %q", i))
		}
		options.Isolation = i
	}

	if runtime.GOOS != "windows" && options.SecurityOpt != nil {
		return nil, errclass.InvalidArgument(errors.Errorf("The daemon on this platform does not support setting security options on build"))
	}
	...
	if ulimitsJSON != "" {
		if err := json.Unmarshal([]byte(ulimitsJSON), &buildUlimits); err != nil {
			return nil, errors.Wrap(errclass.InvalidArgument(err), "error reading ulimit settings")
		}
	...
	if buildArgsJSON != "" {
		...
		if err := json.Unmarshal([]byte(buildArgsJSON), &buildArgs); err != nil {
			return nil, errors.Wrap(errclass.InvalidArgument(err), "error reading build args")
		}
	...
	if labelsJSON != "" {
		...
		if err := json.Unmarshal([]byte(labelsJSON), &labels); err != nil {
			return nil, errors.Wrap(errclass.InvalidArgument(err), "error reading labels")
		}
...
func (br *buildRouter) postBuild(ctx context.Context, w http.ResponseWriter, r *http.Request, vars map[string]string) error {
	...
	if buildOptions.Squash && !br.daemon.HasExperimental() {
		return errclass.InvalidArgument(errors.New("squash is only supported with experimental mode"))
	}
