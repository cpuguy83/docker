...
func (daemon *Daemon) restore() error {
	...
	for _, c := range containers {
		...
		go func(c *container.Container) {
			...
			if !alive {
				// Process will only be nil here if `containerd.Restore` returned an errdefs.NotFoundError
				// In this case we just want to cleanup a dead container from containerd.
				// Later on docker's view of the container state will be updated accorrdingly, i.e. SetStopped.
				if process != nil {
